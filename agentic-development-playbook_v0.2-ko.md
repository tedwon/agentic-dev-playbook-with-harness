# 사람이 방향을 잡고, Agent가 실행한다. - Agentic Development Playbook (Enhanced)
...
```bash
# SpotBugs로 정적 분석 실행 (pom.xml에 설정된 경우)
./mvnw spotbugs:check -q

# OWASP Dependency-Check로 알려진 취약점 확인 (설정된 경우)
./mvnw dependency-check:check -q

# 최소한 보안 관련 코드에서 컴파일 경고가 없는지 확인
./mvnw compile -q 2>&1 | grep -i "warn"
```

전용 보안 스캔 plugin이 아직 설정되지 않았다면 이것은 해결해야 할 갭이에요. 최소한
보안 관련 영역(인증, 입력 검증, 데이터 접근)의 AI 생성 코드를 사람 Code Review 중
추가적인 주의를 기울여 검토하세요.
...
- [ ] 보안 스캔 통과 (SpotBugs, dependency-check, 또는 보안 관련 코드의 수동 리뷰)
