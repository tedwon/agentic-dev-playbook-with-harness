# Humans steer. Agents execute. - Agentic Development Playbook (Enhanced)
...
```bash
# Run SpotBugs for static analysis (if configured in pom.xml)
./mvnw spotbugs:check -q

# Run OWASP Dependency-Check for known vulnerabilities (if configured)
./mvnw dependency-check:check -q

# At minimum, verify no compilation warnings in security-sensitive code
./mvnw compile -q 2>&1 | grep -i "warn"
```

If dedicated security scanning plugins are not yet configured, this is a gap to address.
At minimum, review AI-generated code in security-critical areas (auth, input validation,
data access) with extra scrutiny during human code review.
...
- [ ] Security scanning passes (SpotBugs, dependency-check, or manual review of
